...
void NodeScoreCellOld::update_data(const std::shared_ptr<MATCH_OBJECT>& obj)
{
    obj_ = obj;
...
void NodeScoreCellOld::update_ui()
{
    auto data = obj_.lock();
...
void NodeScoreCellOld::register_download_events()
{
    dispatch_.register_event(EVENT_DOWLOAD_FILE,
                             BIND_FUNC(this, NodeScoreCellOld::handle_download_events));
}
void NodeScoreCellOld::handle_download_events(cmd_data_pointer)
{
    update_ui();
}
