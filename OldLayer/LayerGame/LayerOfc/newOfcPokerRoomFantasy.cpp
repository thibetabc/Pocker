...
bool newOfcPokerRoomFantasy::init()
{
    ...
    root_ = CSLoader::createNode("LayerOfcFantasyPortrait.csb");
    ...
    btn_hand_sort_1_->setTitleText("按花色排序");
    btn_hand_sort_2_->setTitleText("按大小排序");
    btn_hand_sort_3_->setTitleText("按对子排序");;
...
void newOfcPokerRoomFantasy::click_btn_confirm(Ref* sender)
{
    ...
    PDM->send_player_choose_cards(result, ofc_room_data->get_room_id());
}
...
//    std::vector<std::vector<uint32_t>> high_cards;
    ...
    auto flush_sort_func = [](std::vector<uint32_t> vec_one_pair_1, std::vector<uint32_t> vec_one_pair_2){
        uint32_t pair1_big_card = vec_one_pair_1[0];
        uint32_t pair2_big_card = vec_one_pair_2[0];
        for (auto card_iter : vec_one_pair_1)
        {
            if (card_iter > pair1_big_card)
                pair1_big_card = card_iter;
        }
        for (auto card_iter : vec_one_pair_2)
        {
            if (card_iter > pair2_big_card)
                pair2_big_card = card_iter;
        }
