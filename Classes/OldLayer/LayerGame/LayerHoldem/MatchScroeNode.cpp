...
void MatchScroeNode::handle_get_score(const msg::Processor_420_PokerGetScore_DOWN& processor)
{
    ...
    if (subtime.empty())
    {
        if ( processor.roomtype() == msg::RoomType::SIChUANMahjong )
        {
            subtime = "游戏剩余局数：无";
        }
        else
        {
            subtime = tools::local_string("game_time","游戏时长：") +tools::local_string("none","无");
        }
    ...
    else
    {
        if ( processor.roomtype() == msg::RoomType::SIChUANMahjong )
        {
            subtime = "游戏剩余局数：" + subtime ;
        }
        else
        {
            subtime = tools::local_string("game_time","游戏时长：") + subtime;
        }
        
    }
   
    if (processor.roomtype() == msg::RoomType::OFC3 || processor.roomtype() == msg::RoomType::OFC4 || processor.roomtype() == msg::RoomType::SIChUANMahjong)
    {
...
//        btn_close->addClickEventListener(std::bind(&MatchScroeNode::click_btn_close, this, std::placeholders::_1));
        ...
        text_title->setString(tools::local_string("present_records","实时战绩"));
        text_nickname->setString(tools::local_string("nickname","昵称"));
        text_score->setString(tools::local_string("scores","得分"));
        text_current_time->setString(tools::local_string("game_time","游戏时长："));
    ...
    }else
    {
        ...
        auto title_pic = dynamic_cast<Sprite*>(CSLoader::seekNodeByName(root, "word_time_record_5"));
        ...
        text_win->setString(tools::local_string("win_chip", "战绩"));
