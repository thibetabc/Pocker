...
bool PokerRoomLayer::init()
{
    ...
    player_seat_chip_card_positions_.push_back({Point(80, 1060), Point(185, 1025), Point(120, 1068)});
    
    player_seat_chip_card_positions_.push_back({Point(375, 1180), Point(365, 1060), Point(415, 1188)}); // 顶部的位置
    
    player_seat_chip_card_positions_.push_back({Point(670, 1060), Point(560, 1025), Point(610, 1068)});
    ...
    if (!GetRoom()->get_is_record()) {
        uint32_t unread_message_count = 0;//CM->get_unread_message_count(GetRoom()->get_group_id());
    ...
    text_pool_amount_->setFontSize(30); // by ronaldz.
    ...
    image_sit_back_->addClickEventListener([=](Ref*){
        ...
        if(iter != NULL) {
            PDM->send_seat_info(GetRoom()->get_id(), msg::SittingStatus::SITDOWN, iter->get_seat_num());
        }
        else {
            assert(0);
        }
    ...
    auto btn_follow_text=dynamic_cast<Text*>(CSLoader::seekNodeByName(root_, "text_btn_follow"));
    btn_giveup_->setTitleText(tools::local_string("fold","弃牌"));
    btn_watch->setTitleText(tools::local_string("check","看牌"));
    btn_follow_text->setString(tools::local_string("call","跟注"));
    ...
    //Shield Chat Button In GameLayer  by lvzhangle
    btn_chat_->setVisible(false);
    btn_voice_->setPosition(btn_chat_->getPosition());
    
    if(!GameTools::isChineseLanguage())
    {
        change_language(root_);
    }
    
    return true;
}

void PokerRoomLayer::change_language(Node* node)
{
    //text 文本
    std::string text_ch[] =
    {"抓", "暂离", "自动跟注",
    ...
    {"check_box_straddle_text","check_box_sitout_text","check_box_autocall_text",
        "Text_6","check_box_callorfold_text"
    };
    std::string text_en[] =
    {"chatch","leave","auto_call_raise",
        "sit_back","follow_or_give_up"
    };
    printf("the length is %lu",sizeof(text_ch));
    for(int i=0;i<5;i++)
    {
    ...
    if(GameTools::isEnglishLanguage())
    {
         auto check_box_straddle_text = dynamic_cast<Text*>(CSLoader::seekNodeByName(root_, "check_box_straddle_text"));
        check_box_straddle_text->setFontSize(18);
        check_box_straddle_text->setPositionX(80);
        check_box_straddle_text_number->setPositionX(130);
    }
    
    //button 按钮
    std::string btn_text_cn[]=
    {
        "全下","下注","跟注"
    };
    std::string btn_text_name[]=
    {
        "btn_all","btn_chip","btn_add"
    };
    std::string btn_text_en[]=
    {
        "all_in","bet","call"
    };
    for(int i=0;i<3;i++)
    {
        printf("text_name is %s",btn_text_name[i].c_str());
        auto title_setup_new = dynamic_cast<Button*>(CSLoader::seekNodeByName(node, btn_text_name[i]));
        if(NULL !=title_setup_new)
        {
            title_setup_new->setTitleText(tools::local_string(btn_text_en[i],btn_text_cn[i]));
        }
    }
    
    // pic 图片按钮
    std::string btn_root_name[]=//按钮在UI上的名称
    {
        "btn_chat","btn_voice","btn_pause",
        "btn_match_score","btn_buy_chip","btn_record",
    };
    std::string btn_normal_root[]=//按钮普通状态
    {
        "btn_chat_normal.png","btn_voice_normal.png","btn_pause_normal.png",
        "btn_match_point_normal.png","btn_add_chip_normal.png","btn_match_info_normal.png"
    };
    std::string btn_press_root[]=//按钮按下状态
    {
        "btn_chat_press.png","btn_voice_press.png","btn_pause_press.png",
        "btn_match_point_press.png","btn_add_chip_press.png","btn_match_info_press.png"
    };
    for(int i=0;i<6;i++)
    {
        printf("text_name is %s",btn_root_name[i].c_str());
        auto title_setup_new = dynamic_cast<Button*>(CSLoader::seekNodeByName(node, btn_root_name[i]));
        if(NULL !=title_setup_new)
        {
            GameTools::set_pic_change(title_setup_new,btn_normal_root[i],btn_press_root[i]);
        }
    }
}
...
void PokerRoomLayer::set_btn_choice_visible(msg::ChoiceType choice_type_)
{
    switch(choice_type_)
    {
        ...
        case msg::ChoiceType::MuckCard:
            ...
            btn_twice->setTitleText(tools::local_string("muck","盖牌"));
            btn_twice->setTitleFontSize(30);
            
            break;
        case msg::ChoiceType::AskRunTwice:
            btn_twice->setVisible(true);
            btn_twice->setTitleText(tools::local_string("licensing_twice","发两次牌"));
            btn_twice->setTitleFontSize(24);
            
            break;
        case msg::ChoiceType::ReplyRunTwice:
            btn_twice->setVisible(true);
            btn_twice->setTitleText(tools::local_string("agrees_license_twice","同意发两次牌"));
            btn_twice_2->setVisible(true);
            btn_twice_2->setTitleText(tools::local_string("disagrees_license_twice","不同意发两次牌"));
...
{
    PDM->send_get_score(this->GetRoom()->get_id());
}
...
void PokerRoomLayer::click_btn_share(Ref* sender)
{
    ...
    layerShare->setShareGamePokerRoomInfo(uintUserID, uintGroupID, uintRoomID);
...
void PokerRoomLayer::click_btn_buy_chip(Ref* sender)
{
    ...
    if(user) {
        ...
        uint32_t buyin_min = GetRoom()->get_buyin_min() - user->get_properties()->chips();
        min_chip = buyin_max - user->get_properties()->chips();
        if(min_chip <= 0) {
...
{
    PDM->send_poker_choice(GetRoom()->get_id(), choicetype, true);
    btn_twice->setVisible(false);
    btn_twice->setVisible(false);
}
void PokerRoomLayer::click_btn_twice_2(Ref *sender)
{
    PDM->send_poker_choice(GetRoom()->get_id(), choicetype, false);
    btn_twice_2->setVisible(false);
    btn_twice_2->setVisible(false);
}
...
void PokerRoomLayer::click_btn_giveup(Ref* sender)
{
    ...
    PDM->send_poker_player_game_action(GetRoom()->get_id(), msg::ActionStatus::FOLD, 0);
...
void PokerRoomLayer::click_btn_watch(Ref* sender)
{
    clear_view(sender);
    hidden_action_btns();
    PDM->send_poker_player_game_action(GetRoom()->get_id(), msg::ActionStatus::CHECK, 0);
}
...
void PokerRoomLayer::click_btn_chip(Ref* sender)
{
    ...
    if(!node_chip_->isVisible()) {
        ...
        node_chip_->setVisible(false);
        hidden_action_btns();
        PDM->send_poker_player_game_action(GetRoom()->get_id(), msg::ActionStatus::BET, node_chip_->get_amount());
    }
}
...
void PokerRoomLayer::click_btn_follow(Ref* sender)
{
    ...
    hidden_action_btns();
    node_chip_->setVisible(false);
    PDM->send_poker_player_game_action(GetRoom()->get_id(), msg::ActionStatus::CALL, 0);
}
...
void PokerRoomLayer::click_btn_add(Ref* sender)
{
    ...
    if(!node_chip_->isVisible()) {
        ...
        node_chip_->setVisible(false);
        hidden_action_btns();
        PDM->send_poker_player_game_action(GetRoom()->get_id(), player_operator_action_.actionchoice() == msg::ActionChoice::FOLD_CHECK_BET ? msg::ActionStatus::BET : msg::ActionStatus::RAISE, node_chip_->get_amount());
    }
}
void PokerRoomLayer::click_btn_all(Ref* sender)
{
    ...
    uint32_t amount = user->get_properties()->chips();
    PDM->send_poker_player_game_action(GetRoom()->get_id(), msg::ActionStatus::ALLIN, amount);
    hidden_action_btns();
}
...
void PokerRoomLayer::click_btn_voice(Ref* sender, Widget::TouchEventType event)
{
    ...
    switch (event) {
        ...
        case cocos2d::ui::Widget::TouchEventType::ENDED: {
            ...
            if(record_time != 0) {
                PDM->send_game_voice(record_filename_, record_time, GetRoom()->get_id());
                auto &poker_room_data = this->GetRoom();
                poker_room_data->add_owner_last_voice(GDM->get_user_id(), record_filename_+".amr", record_time);
            }
            else {
...
{
    uint32_t roomID = GetRoom()->get_id();
...
void PokerRoomLayer::update_show_card()
{
    ...
    if (user) {
        ...
        GetRoom()->set_show_card(showCards);
...
void PokerRoomLayer::PositionUpdate()
{
    ...
    for (auto& iter : info_nodes_) {
        ...
        for (auto& it : users)
        {
            if(it.second->get_seat_num() == iter.first &&
        ...
        if(!is_exist) {
            ...
            if(it != users.end() && (it->second->get_status() != msg::SittingStatus::OBSERVER && it->second->get_status() != msg::SittingStatus::LEAVE)) {
...
{
    uint32_t user_id = data.actionrequest().actionplayerid();
    ...
    if(info_node) {
        info_node->show_progress_box(data.actionrequest().countdown(),0, data.actionrequest().isusingtimebank());
    }
}
...
void PokerRoomLayer::handle_game_hand_info(const msg::HandResultInfo& data)
{
    ...
    for (auto& it : info_nodes_) {
        it.second->show_green_box(false);
        it.second->set_description("");  // we clear the action_hint. // by ronaldz. 20160606
        ...
        for (int j = 0; j < data.pot(i).playerwin_size(); j++)
        {
            ...
            if(chip != 0)
            {
                ...
                if(info)
                {
                    ...
                    table_pot_chips_[i]->setVisible(false);  // by ronaldz. we remove this side pot.
                    ...
                    MoveTo* move = MoveTo::create (1.0f, info->getPosition() +  info->get_text_point());
...
void PokerRoomLayer::update_user_property(const std::shared_ptr<user_texas>& user)
{
    ...
    switch (user->get_properties()->actionstatus()) {
        ...
        case msg::ActionStatus::ALLIN:
        {
            ...
            if (user->get_id() == GDM->get_user_id()) {
                ...
                int betchips = user->get_properties()->betchips();
                int haschip = user->get_properties()->chips();
    ...
    _last_chip_num = user->get_properties()->chips();
...
const PokerRoomLayer::POINTS PokerRoomLayer::get_position_with_seat_num(const uint32_t seat_num)
{
    ...
    if(!isRecord && iter_this_user != users.end() && iter_this_user->second->get_status() != msg::SittingStatus::OBSERVER && iter_this_user->second->get_seat_num() > 0) {
...
void PokerRoomLayer::add_player_info_layer(std::shared_ptr<user_texas> user)
{
    ...
    user_info->set_player_info(info, group_id,owner_id,room_id);
}
void PokerRoomLayer::init_seats()
{
    ...
    for (auto& iter : points) {
        ...
        info->set_click_photo_callback(std::bind(&PokerRoomLayer::add_player_info_layer,this,std::placeholders::_1));
...
void PokerRoomLayer::register_events()
{
    ...
    dispatch_.register_event(EVENT_HIT_NOT_ENOUGH_CALL,BIND_FUNC(this, PokerRoomLayer::add_Shop_Layer));
}
...
//    log("进入pokerRoomLayer游戏---删除图片资源前: %s", Director::getInstance()->getTextureCache()->getCachedTextureInfo().c_str());
    Director::getInstance()->purgeCachedData();
    Director::getInstance()->getTextureCache()->removeAllTextures();
//    log("进入pokerRoomLayer游戏---删除图片资源后: %s", Director::getInstance()->getTextureCache()->getCachedTextureInfo().c_str());
...
void PokerRoomLayer::add_Shop_Layer(cmd_data_pointer data)
{
    LayerShop* shop = LayerShop::create(false);
    this->addChild(shop,TAG_BUY_CHIP+1,"LayerShop");
}
