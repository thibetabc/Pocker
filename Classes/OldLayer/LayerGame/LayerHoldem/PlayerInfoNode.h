...
public:
    ...
    std::string change_status_language(const std::string& str);
    void set_click_photo_callback(const std::function<void(std::shared_ptr<user_texas>)>& callback) { click_photo_callback_ = callback; };
...
private:
    ...
    Text* text_player_info_bet_;
    Text* text_player_info_check_;
    Text* text_player_info_fold_;
    Text* text_player_info_raise_;
    ...
    std::function<void(std::shared_ptr<user_texas>)> click_photo_callback_;
