...
void PlayerInfoNode::update_data(const std::shared_ptr<user_texas>& user)
{
    ...
    text_chip_amount_->setString(tools::to_string(user_->get_properties()->chips()));
    sprite_chip_->setTexture(GameTools::get_chip_image_name(user_->get_properties()->chips()));
    ...
    switch (user->get_status()) {
        ...
        case msg::SittingStatus::OBSERVER:
            set_description(tools::local_string("standing_up","站起"));
            break;
        case msg::SittingStatus::SITOUT:
            set_description(tools::local_string("leave","暂离"));
            break;
        case msg::SittingStatus::SITDOWN:
            set_description(tools::local_string("wait","等待"));
            break;
        default:
            set_description("");
            break;
...
//        
//        uint32_t group_id = poker_room_data->get_group_id();
...
std::string PlayerInfoNode::change_status_language(const std::string& str)
{
    std::string description;
   
    if(str == "让牌")
    {
    description = tools::local_string("check","让牌");
    }
    else if(str == "下注")
    {
        description = tools::local_string("bet","下注");
    }
    else if(str == "加注")
    {
        description = tools::local_string("raise","跟注");
    }
    else if(str == "全下")
    {
        description = tools::local_string("all_in","全下");
    }
    else if(str == "暂离")
    {
        description = tools::local_string("leave","暂离");
    }
    else if(str == "等待")
    {
        description = tools::local_string("wait","等待");
    }
    else if(str == "站起")
    {
        description = tools::local_string("standing_up","站起");
    }
    else if(str == "弃牌")
    {
        description = tools::local_string("fold","弃牌");
    }
    else if(str == "跟注")
    {
        description = tools::local_string("raise","跟注");
...
void PlayerInfoNode::set_description(const std::string& description)
{
    ...
    if (description.empty() && user_ != nullptr)
    {
        ...
        {
            std::string showstatus=change_status_language(description);
            log("status123:%s",showstatus.c_str());
            text_status_->setString(showstatus);
        }
    }
    else
    {
        showstatus = change_status_language(description);
        log("status456:%s",showstatus.c_str());
        text_status_->setString(showstatus);
    }
    ...
    { // ronaldz. 最下面的需要吧夹住这些内容上移一下。
       player_info_fold->setPositionY(70+player_info_fold->getContentSize().height*1.5);
        ...
        player_info_allin->setPositionY(70+player_info_allin->getContentSize().height*1.2);
    }
    ...
    if(getPositionY() > -1)// 原来是200， 这个是有很大问题的， 也就是说谁坐在最下面谁就不变灰。 这是不对的。 我改成－1. 也就是， 无论谁都要改。  ronaldz 20160220
    {
        ...
        printf("prensent description is %s",showstatus.c_str());
        
        if (showstatus == tools::local_string("fold","弃牌")) {
            if (getPositionX() >= 375) {
                player_info_fold->setTexture("login_res/game_res/player_info_fold_1.png");
            }
            text_player_info_fold_->setString(showstatus);
        ...
        else if (showstatus == tools::local_string("leave","暂离") || description ==tools::local_string("wait","等待"))
        {
            if (sprite_icon_) {
                sprite_icon_->setColor(Color3B::GRAY);
            }
            this->setWholeOpacity(100);
        }else if (showstatus =="" || description ==tools::local_string("standing_up","站起"))
        {
            this->setWholeOpacity(255);
        }
...
//    AM->playEffect("sound/oh.mp3");
    ...
    text_win_amount_->setFontName("HelveticaNeue-Bold");
    text_win_amount_->setFontSize(50);
   // text_win_amount_->setColor(Color3B::RED);
    text_win_amount_->setGlobalZOrder(30000);
    FiniteTimeAction * mb = MoveBy::create(1, Vec2(0,190));
...
//    Sequence::create(action, CCCallFunc::create(<#const std::function<void ()> &func#>),NULL)
    ...
    text_win_amount_->setAnchorPoint(Point(1, 0.5));
    
    //text_win_amount_->setPosition(Point(text_chip_amount_->getPosition().x , text_chip_amount_->getPosition().y));
...
void PlayerInfoNode::recreate()
{
    ...
    sprite_winner_->setPositionY(-90);   // by ronaldz. 我们往下放一点， 让牌露出来。把牌型提示也显示出来。
    ...
    text_win_type_->setString(tools::local_string("one_pair","一对"));
    ...
    text_player_info_bet_ = dynamic_cast<Text*>(CSLoader::seekNodeByName(player_info_bet, "player_info_bet_text"));
    
    player_info_check = dynamic_cast<Sprite*>(CSLoader::seekNodeByName(root, "player_info_check"));
    player_info_check->setVisible(false);
    text_player_info_check_ = dynamic_cast<Text*>(CSLoader::seekNodeByName(player_info_check, "player_info_check_text"));
    
    player_info_fold = dynamic_cast<Sprite*>(CSLoader::seekNodeByName(root, "player_info_fold"));
    player_info_fold->setVisible(false);
    text_player_info_fold_ = dynamic_cast<Text*>(CSLoader::seekNodeByName(player_info_fold, "player_info_fold_text"));
    
    player_info_raise = dynamic_cast<Sprite*>(CSLoader::seekNodeByName(root, "player_info_raise"));
    player_info_raise->setVisible(false);
    text_player_info_raise_ = dynamic_cast<Text*>(CSLoader::seekNodeByName(player_info_raise, "player_info_raise_text"));
    ...
    text_nickname_->setFontSize(30);  // by ronaldz
    text_chip_amount_->setFontSize(30);   // by ronaldz.
    sprite_chip_->setVisible(false);// by ronaldz.
...
void PlayerInfoNode::click_btn_kick(Ref* sender)
{
    ...
    if (user_)
    {
            ...
            !is_tournament)
        {
            ...
            auto tip = TipView::showAlertView(tools::local_string("kick_off_players","确定要踢走该玩家吗？"));
            tip->show_cancel(true);
            tip->set_tipview_close_callback([this](){
                PDM->send_kick_player(GetRoom()->get_id(), user_->get_id());
